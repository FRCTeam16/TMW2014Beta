...
//	autoChooser->AddObject("05. Shoot 2 From Side", (void*)fire2Side);
	...
	cout<< "FrontLeft Firmware Version " << driveTrain->frontLeftSteer->GetFirmwareVersion() << endl
	<< "FrontRight Firmware Version " << driveTrain->frontRightSteer->GetFirmwareVersion() << endl
	<< "RearLeft Firmware Version " << driveTrain->rearLeftSteer->GetFirmwareVersion() << endl
	<< "RearRight Firmware Version " << driveTrain->rearRightSteer->GetFirmwareVersion() << endl
//	<< "CamLeft Firmware Version " << shooter->camLeft->GetFirmwareVersion() << endl
//	<< "CamRight Firmware Version " << shooter->camRight->GetFirmwareVersion() << endl
//	<< "BeaterBar Firmware Version " << pickup->beaterBar->GetFirmwareVersion() << endl
	<< "Starting CheesyVision Initiation" << endl;
...
void Robot::AutonomousInit() {
	...
	case fire2DriveForward: case fire2DriveForwardWide:
		...
		//genericAutoProgram.push_back(JiggleBall);
		//genericAutoProgram.push_back(DropPickup);
		genericAutoProgram.push_back(Fire);
		genericAutoProgram.push_back(End);		
		break;
...
//		LEDSet(4);
	...
	case ResetShooterAndCollect:
		...
		SmartDashboard::PutString("AutoStep", "ResetShooterAndCollect");
		cout << "ResetShooterAndCollect" << endl;
		if(shooter->GetResetCamComplete() && (ballCollectedInc > 40 || autoStepTimer->HasPeriodPassed(1.5))) {
	...
	case FirstResetShooter:
		...
		if(turnDirection == 0) {
			if(cheeseView->GetLeftStatus() || autoStepTimer->HasPeriodPassed(2.0)) {
				turnDirection = -1;
			}
			else if (cheeseView->GetRightStatus()) {
				turnDirection = 1;
			}
		}
		
		SmartDashboard::PutString("AutoStep", "FirstResetShooter");
		cout << "FirstResetShooter" << endl;
	...
	case WaitToFire50:
		x = 0;
		y = 0;
		SmartDashboard::PutString("AutoStep", "WaitToFire50");
		cout << "WaitToFire50" << endl;
	...
	case WaitToFire75:
		x = 0;
		y = 0;
		SmartDashboard::PutString("AutoStep", "WaitToFire75");
		cout << "WaitToFire75" << endl;
		if(autonomousTimer->HasPeriodPassed(7.5))
			autoStepComplete = true;
		break;
		
	case WaitForHot:
		if(cheeseView->GetLeftStatus() || cheeseView->GetRightStatus())
			autoStepComplete = true;
		SmartDashboard::PutString("AutoStep", "WaitForHot");
		cout << "WaitForHot" << endl;
		break;
	...
	case DriveForwardFirstTurn:
		...
		if(turnDirection == 0) {
			if(cheeseView->GetLeftStatus() || autoStepTimer->HasPeriodPassed(2.5)) {
				turnDirection = -1;
			}
			else if (cheeseView->GetRightStatus()) {
				turnDirection = 1;
		...
		cout << "DriveForwardFirstTurn" << endl;
		
		if(turnDirection != 0 && (cheeseView->GetRightStatus() && cheeseView->GetLeftStatus())) {
			turnDegree = 20;
		}
		else
			turnDegree = 10;
		...
		if(turnDirection != 0 && onTargetTimer->HasPeriodPassed(.1) && shooter->GetResetCamComplete()){
			autoStepComplete = true;
			pickup->beaterBar->Set(0);
			x = 0;
			y = 0;
	...
	case SecondTurn:
		...
		cout << "SecondTurn" << endl;
		
		if((cheeseView->GetRightStatus() && cheeseView->GetLeftStatus())) {
			turnDegree = 20;
		}
		else
			turnDegree = 10;
		...
		if(onTargetTimer->HasPeriodPassed(.1) && shooter->GetResetCamComplete()){
			autoStepComplete = true;
			pickup->beaterBar->Set(0);
		}
		break;
	...
	case LoadBall:
		...
		cout << "LoadBall" << endl;
		if(autoStepTimer->HasPeriodPassed(.6)){
	...
	case JiggleBall:
		x = 0;
		y = -1.0;
		SmartDashboard::PutString("AutoStep", "JiggleBall");
		cout << "JiggleBall" << endl;
		if(autoStepTimer->HasPeriodPassed(.4)) {
			x=0;
			y=0;
		}				
		if(autoStepTimer->HasPeriodPassed(.8)) {
			autoStepComplete = true;
			pickup->beaterBar->Set(0);
		}
		break;
...
/******************SHOOTER**************************************/
	...
	if(oi->getGamePad()->GetRawButton(10)){
			shooter->RunCams(-oi->getGamePad()->GetRawAxis(1), true);
	}
	else if(oi->getDriverJoystickRight()->GetRawButton(5)){
		shooter->RelieveStress();
	}
	else if(oi->getDriverJoystickRight()->GetRawButton(3)){
		shooter->Reset();
	}
	else {
		shooter->CamChecker(); //Runs every cycle to control cam postion
		if (oi->getDriverJoystickRight()->GetRawButton(2)) {
			shooter->Fire(0,false);
		}
	}
...
//	}
//	else
	pickup->beaterBar->Set(oi->getGamePad()->GetRawAxis(3));
...
//	pickup->beaterBar->UpdateSyncGroup(0x00000001);
...
//	SmartDashboard::PutBoolean("12-CamRightJagAlive",shooter->camRight->IsAlive());
	...
	SmartDashboard::PutNumber("06-FLSteerFaultsInt",(float) driveTrain->frontLeftSteer->GetFaults());
	SmartDashboard::PutNumber("07-FRSteerFaultsInt",(float) driveTrain->frontRightSteer->GetFaults());
	SmartDashboard::PutNumber("08-RLSteerFaultsInt",(float) driveTrain->rearLeftSteer->GetFaults());
	SmartDashboard::PutNumber("09-RRSteerFaultsInt",(float) driveTrain->rearRightSteer->GetFaults());
//	SmartDashboard::PutNumber("11-CamLeftFaultsInt",shooter->camLeft->GetFaults());
//	SmartDashboard::PutNumber("12-CamRightFaultsInt",shooter->camRight->GetFaults());
	SmartDashboard::PutNumber("13-BeaterBarFaultsInt",pickup->beaterBar->GetFaults());
	SmartDashboard::PutNumber("GamePadAxis1",oi->getGamePad()->GetRawAxis(1));
	SmartDashboard::PutNumber("GamePadAxis0",oi->getGamePad()->GetRawAxis(0));
	//AutoInfo
...
bool Robot::KinectRightSelect() {
	return true; //kinect->GetSkeleton().GetWristRight().y > kinect->GetSkeleton().GetShoulderRight().y;
}
bool Robot::KinectLeftSelect() {
	return false; //kinect->GetSkeleton().GetWristLeft().y > kinect->GetSkeleton().GetShoulderLeft().y;
}
